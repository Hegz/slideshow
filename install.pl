...
if ($install) {
	...
	unless ($installrdy) {
		print
			"Please edit the config file options.pm to ensure that eveything is setup correctly.\n";
		print "Install Aborting, Unsuccessful\n";
		exit;
	}
	unless ($machine) {
		print
			"You will need to set the machine dns name for the slideshow to run.\n";
		print "Install Aborting, Unsuccessful\n";
		exit;
	}
	...
	unless ( $ENV{'USER'} eq 'root' ) {
		print
			"This script requires root privilages.  Please run with sudo.\n";
		print "Install Aborting, Unsuccessful\n";
		exit;
	}
	...
	open my $passwdfile, '<', '/etc/passwd'
		or die("error opening passwd file $!\n");
	while (<$passwdfile>) {
		if (m/$showuser/) {
			print
				"the user account $showuser is already active on the system.  Is the slideshow account already setup?\n";
			print "Install Aborting, Unsuccessful\n";
			exit;
		}
	}
	...
	opendir my $source, "$currentdir"
		or die("Error opening current directory $!\n");
	...
	unless ($correctfolder) {
		print
			"This script must be run from within extracted file directory.\n";
		print
			"Please change to the directoy where you extracted the files and try again\n";
		print "Install Aborting, Unsuccessful\n";
		exit;
	}
	...
	while (<$autologin>) {
		if (m/(.*)\:$showuser/) {
			print
				"Account $showuser is already set to atologin on machine $1\n";
			print "Install Aborting, Unsuccessful\n";
			exit;
		}
	}
	...
	open $autologin, '>>', $autologinfile
		or die(
		"Error opening autologin file for write ($autologinfile):$!\nInstall Aborting, Unsuccessful\n"
		);
	...
	# Update the cron scripts for the correct paths
	{
		local $^I   = '.bak';
		local @ARGV = (
			"$userhomedir/.kde/Autostart/cron-15min.sh",
			"$userhomedir/.kde/Autostart/cron-1min.sh"
		);
		while (<>) {
			s/PATH/$userhomedir/;
			s/LOG/$logdir/;
		...
		unlink(
			"$userhomedir/.kde/Autostart/cron-15min.sh.bak",
			"$userhomedir/.kde/Autostart/cron-1min.sh.bak"
		);
	}
	...
	print
		"Please restart the slideshow machine ($machine) to complete the installation\n";
	exit;
}
elsif ($uninstall) {
	...
	if ( $currentdir ne $userhomedir ) {
		print
			"Please run the uninstall script from the Slidshow users home folder\n";
	...
	print
		"Please restart the slideshow machine ($machine) to complete the Uninstallation\n";
	exit;
}
elsif ($update) {
