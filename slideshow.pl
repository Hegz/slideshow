...
open my $log, '>>', "$showlogfile"
	or die("Cannot open log file ($showlogfile). $!");
opendir my $show, $showdir
	or die("Cannot Open Slide show Directory ($showdir). $!");
my $mytime = gettime();
{
				...
				if (m/$magicvideo\.$ext/) {
					...
					if (`ps -a u -u $showuser | grep -v grep | grep -c $magicvideo `
						...
						if ( ( stat("$showdir/$magicvideo") )[9]
							...
							print $log
								"[M] $mytime Updated $magicvideo mplayer video file.\n";
							unless ( fork() ) {
								exec
									"$mplaybin -fs -quiet $convertdir/$magicvideo -loop 0 &> /dev/null";
							}
							exit 0;
						}
						else {
						...
						start_show($magicvideo);
						print $log
							"[M] $mytime Started $magicvideo mplayer video file.\n";
						unless ( fork() ) {
							exec
								"$mplaybin -fs -quiet $convertdir/$magicvideo -loop 0 &> /dev/null";
						}
...
if ( ( ( stat("$userhomedir/options.pm") )[9] )
	!= ( ( stat("$showdir/README.txt") )[9] ) ) {
